...
export const apis = {${r}
};`;await c(e,o,"_apis.ts")}async function U(e,t){let r="";for(let o of t){let n=o.replace(e,"").replace("/+page.tsx","").replace("/+page.ts","").replace("/+page.vue",""),i=o.replace(e,".").replace(".tsx","").replace(".ts","").replace(".vue",""),p=o.replace(e+"/","").replace("/+page.tsx","").replace("/+page.ts","").replace("/+page.vue","").split("/").join("_");p.indexOf("+page.")>-1&&(p="_"),r+=`
  ${p}: {
    path: "${n}",
    render: () => import("${i}"),
  },`}let s=`// Don't edit
...
`;await c(e,o,"_serves.ts")}var u=!1;async function d(e){if(u)return;u=!0;let[t,r]=await Promise.all([(0,y.default)([`${e}/**/+page.(tsx|ts|vue)`]),(0,y.default)([`${e}/**/+serve.ts`])]);await Promise.all([t.length&&U(e,t),r.length&&S(e,r),r.length&&w(e,r)].filter(Boolean)),u=!1}var D=/(\+page|\+serve)/;async function z(e,t=!1){if(d(e),!t){console.log("loaded glob-router");return}(0,A.watch)(e,{persistent:!0}).on("add",r=>{D.test(r)&&d(e)}).on("change",r=>{D.test(r)&&d(e)})}var K=z;module.exports=k(M);0&&(module.exports={});
